...
$ cargo run part-one-input.txt
   Compiling aoc-2019-day-2 v0.1.0 (C:\src\advent-of-code\2019\day-2)
    Finished dev [unoptimized + debuginfo] target(s) in 2.80s
     Running `C:\src\advent-of-code\2019\day-2\target\debug\aoc-2019-day-2.exe part-one-input.txt`
Parsing file: part-one-input.txt
End state: 3562672
...
until you add more instructions to the computer, this is always 4 (1 opcode + 3 parameters) for the add and
multiply instructions. (The halt instruction would increase the instruction pointer by 1, but it halts the
program instead.)
...
impl<'a> ProgramState<'a> {
    ...
    pub fn patch_noun_and_verb(&mut self, noun: u32, verb: u32) {
        self.state[1] = noun;
        self.state[2] = verb 
    }
...
fn main() {
    ...
    if args.len() == 2 {
        ...
        let mut instruction = instructions.clone()
        let mut state = ProgramState::from_vec(&mut instruction);
        ...
        */
        ...
        // Part Two
        let instructions = process_file(filename);

        for noun in 0..152 {
            for verb in 0..152 {

                let mut instruction_clone = instructions.clone();
                let mut state = ProgramState::from_vec(&mut instruction_clone);

                // Patch state as instructed above.
                state.patch_noun_and_verb(noun, verb);

                // Execute program
                state.execute();

                if instruction_clone[0] == 19690720 {
